...
var envPath = fs.existsSync(localEnv) ? localEnv : path.join(rootDir, process.env.NODE_ENV === "production" ? ".env.production" : ".env");
...
var CONFIG = {
  ...
  logsDir: fs.existsSync(path.join(__dirname, "logs")) ? path.join(__dirname, "logs") : path.resolve(rootDir, "logs"),
  audioAssets: {
...
// src/logger.js
var { logsDir } = CONFIG;
var errorLogPath = path2.join(logsDir, "error.log");
var combinedLogPath = path2.join(logsDir, "combined.log");
var logger = import_winston.default.createLogger({
  ...
  transports: [
    new import_winston.default.transports.File({ filename: errorLogPath, level: "error" }),
    new import_winston.default.transports.File({ filename: combinedLogPath }),
...
async function processZipFile(filePath) {
  try {
    ...
    logger_default.info(`Processing file: ${fileName}`);
    let hasNonDocxFiles = false;
    let processedFiles = 0;
    let processedDocxCount = 0;
    await fs2.mkdir(CONFIG.outputFolder, { recursive: true });
    const zip = new import_adm_zip.default(filePath);
    for (const entry of zip.getEntries()) {
      const extension = path3.extname(entry.entryName).toLowerCase();
      if (extension === ".docx") {
        const outputPath = path3.join(CONFIG.outputFolder, entry.entryName);
        zip.extractEntryTo(entry, CONFIG.outputFolder, false, true);
        logger_default.info(`Extracted .docx file: ${entry.entryName} to ${outputPath}`);
        processedFiles++;
        processedDocxCount++;
      } else {
        const extractDir = path3.dirname(filePath);
        zip.extractEntryTo(entry, extractDir, false, true);
        logger_default.info(`Extracted non-docx file: ${entry.entryName} to ${extractDir}`);
        hasNonDocxFiles = true;
        processedFiles++;
      }
    }
    await fs2.unlink(filePath);
    logger_default.info("File deleted successfully");
    return {
      success: true,
      hasNonDocxFiles,
      filename: fileName,
    ...
    logger_default.error(`Error processing file ${filePath}: ${error.message}`);
    throw error;
  }
}
...
function playNotificationSound(type) {
  ...
  if (!sounds[type]) {
    logger_default.error(`Invalid sound type: ${type}`);
    return;
  }
  ...
  exec(command, (err) => {
    if (err) {
      logger_default.error(`Error playing ${type} sound: ${err}`);
    } else {
      logger_default.info(`Played ${type} notification sound
`);
    }
  });
}
...
logger_default.info(`zipFilePath: ${zipFilePath}`);
var main = async () => {
  try {
    validateConfig();
    if (!zipFilePath) {
      logger_default.error("No ZIP file path provided.");
      process.exit(1);
    }
    const result = await processZipFile(zipFilePath);
    logger_default.info("Success!");
...
*/
//# sourceMappingURL=unzip-wendy-bundle.mjs.map
